...
class User:
    def __init__(self, name, surname, posts, location):
        ...
        self.coords: list = self.get_coordinates()
        self.marker: object = map_widget.set_marker(self.coords[0], self.coords[1], text=f"{self.location}")
...
def update_user(i) -> None:
    ...
    users[i].coords = users[i].coords.get_coordinates()
    users[i].marker.delete()
    users[i].marker = map_widget.set_marker(users[i].coords[0], users[i].coords[1], text=f"{users[i].location}")
